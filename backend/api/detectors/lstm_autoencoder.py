...
from detectors.isolation_forest import FEATURE_ORDER, vectorize  # share feature order
    ...
    def train(
        ...
        *,
        ...
        for ep in range(epochs):
            ...
            if wandb_run is not None and epoch_losses:
                wandb_run.log(
                    {
                        "lstm/epoch": ep + 1,
                        "lstm/train_loss": sum(epoch_losses) / len(epoch_losses),
                    }
                )
        ...
        if wandb_run is not None:
            wandb_run.log({"lstm/recon_mse_p99": threshold, "lstm/epochs": epochs})
